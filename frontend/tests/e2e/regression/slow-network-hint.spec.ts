...
import { joinSender, openClipboardApp, senderStatus, waitForText } from "../helpers/clipboardApp";
...
test("shows a slow-network warning before timing out when a receiver joins under a network drop", async ({
  browser,
}, testInfo) => {
  const roomId = `e2e-slow-network-${Date.now()}`;
  ...
  try {
    ...
    const observedWarning =
      NETWORK_WARNING_TEXTS.find((text) => receiverPanelTextAfterWarning.includes(text)) ??
      null;
    ...
    await expect
      ...
      .not.toContain(NETWORK_WARNING_TEXTS[0]);

    await expect
      .poll(async () => (await receiverPanel.textContent()) ?? "", {
        timeout: 20_000,
      })
      .not.toContain(NETWORK_WARNING_TEXTS[1]);
