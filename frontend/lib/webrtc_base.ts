...
export default class BaseWebRTC {
  ...
  protected log(
    ...
    logWithLegacyLevel(this.constructor.name, level, message, context);
  }
  ...
  protected handleConnectionStateChange(
    ...
    const stateHandlers = {
      ...
      disconnected: async () => {
        ...
        logger.debug("Peer connection disconnected", {
          peerId,
          isInitiator: this.isInitiator,
        });
  ...
  protected setupDataChannel(
    ...
    dataChannel.onclose = () => {
      logger.debug("Data channel closed", { peerId });
  ...
  public async joinRoom(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const resolveAsJoined = (reason: string) => {
        ...
        logger.debug("Early join completed", {
      ...
      const joinResponseHandler = (response: JoinRoomResponse) => {
        ...
        if (response.success) {
          ...
          logger.debug("Join room succeeded", {
            peerId: this.socket.id,
          ...
          this.isInRoom = false;
          this.roomId = null;
          logger.warn("Join room failed", { roomId, message: response.message });
          if (!this.reconnectionInProgress) {
            this.onLifecycleEvent?.({
              type: "join_failed",
              roomId,
