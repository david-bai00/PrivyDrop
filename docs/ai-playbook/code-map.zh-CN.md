...
- `frontend/hooks/` — 业务逻辑中枢（React Hooks）。
  ...
  - `frontend/hooks/useWebRTCConnection.ts` — WebRTC 生命周期与编排 API；初始化 `WebRTCStoreCoordinator`，广播入口已收敛到 coordinator command，不再由 hook 自己读取 sender payload 组装发送参数，也不再暴露 `sender/receiver` 内部实例；对外仍保留原有 `broadcastDataToAllPeers` 风格接口，但内部已切到“publish draft 并广播 published”语义。重置连接时直接走 `shutdownSender("leave_room")` / `shutdownReceiver("leave_room")`。
