...
- **配置与构建**
  ...
  - `frontend/tests/unit/*.test.ts` — 最小自动化护栏，当前覆盖 lifecycle 规则、async send 结果语义、`Map` 广播/cleanup 规则、接收状态机、ChunkProcessor 封包解析/校验边界、sender/receiver shutdown 策略与 store reset 边界。
